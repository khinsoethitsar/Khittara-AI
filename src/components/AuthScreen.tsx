...
export default function AuthScreen() {
  ...
  const handleAuthError = (err: any) => {
    ...
    if (err.code === 'auth/popup-closed-by-user') {
      message = lang === 'my'
        ? "အစ်ကို Login Window ကို ပိတ်လိုက်တဲ့အတွက် (ဒါမှမဟုတ် Browser က Block လိုက်တဲ့အတွက်) အကောင့်ဝင်လို့ မရပါဘူးရှင်။ Iframe restrictions ကြောင့် Popup အလိုအလျောက်ပိတ်သွားတတ်လို့ အကောင်းဆုံးကတော့ အောက်က 'Open in New Tab' ခလုတ်လေးကိုနှိပ်ပြီး Tab အသစ်မှာ Login ဝင်ကြည့်ပါနော်။ ✨💖"
        : "The sign-in popup was closed before completion. This often happens inside an iframe. Please use the 'Open in New Tab' button below to sign in more reliably.";
          ...
          <div className="space-y-4">
            {isInIframe && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                className="mb-6 p-4 rounded-3xl bg-amber-500/10 border border-amber-500/20 glass-panel"
              >
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-[11px] leading-relaxed text-amber-200/90 font-medium">
                      {lang === 'my' 
                        ? "အစ်ကိုရေ၊ အခု App က iframe ထဲမှာ ရှိနေလို့ Login Popup ပိတ်သွားတာမျိုး ကြုံရနိုင်ပါတယ်ရှင်။ အဆင်မပြေရင် Tab အသစ်မှာ ဖွင့်ပြီး Login ဝင်ပေးပါနော်။ ✨"
                        : "You're in a preview iframe. Sign-in popups might be restricted. If sign-in fails, please try opening in a new tab."}
                    </p>
                    <button 
                      onClick={openInNewTab}
                      className="mt-3 flex items-center gap-2 text-[10px] font-bold text-amber-500 hover:text-amber-400 decoration-amber-500/30 transition-all uppercase tracking-wider bg-amber-500/20 px-3 py-1.5 rounded-full"
                    >
                      <Globe className="w-3 h-3" />
                      {lang === 'my' ? "Tab အသစ်တွင် ဖွင့်ရန် ↗" : "Open in New Tab ↗"}
                    </button>
                  </div>
                </div>
              </motion.div>
              ...
              {error && (
                ...
                >
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    {(error.includes("Tab") || error.includes("window")) && (
                      <button 
                        onClick={openInNewTab}
